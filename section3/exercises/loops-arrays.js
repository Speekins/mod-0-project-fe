...
for (let currentAnimal = 0; currentAnimal < animals.length; currentAnimal++) {
  console.log(`The ${animals[currentAnimal]} is awesome!`);
}


// YOU DO: Create an array of foods and then iterate over that array to log "Add
// <food> to shopping list" for each food item.
let foodsToGet = ['Cinnamon Toast Crunch', 'Spaghetti', 'Pears', 'Animal Crackers', 'Hummus'];
for (let i = 0; i < foodsToGet.length; i++) {
  console.log(`Add ${foodsToGet[i]} to the shopping list.`);
}

// YOU DO: Create an array of numbers and then iterate over that array to log double
// of each of the number. (Ex: 4 doubled is 8.)
let numbers = [7, 8, 2, 14, 20, 3, 9, 44];

for (let i = 0; i < numbers.length; i++) {
  let doubled = numbers[i] * 2;
  console.log(`${numbers[i]} doubled is ${doubled}.`)
}
